...
def test_reverse_single_passes_optimize_to_loop(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "re_agent.reverse.orchestrator.single.run_fix_loop",
        fake_run_fix_loop,
    )
...
def test_reverse_single_respects_optimize_false(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = ReAgentConfig.create_default()
    config.reverse.orchestrator.optimize = False
    config.reverse.output.report_dir = str(tmp_path / "reports")
    config.reverse.output.log_dir = str(tmp_path / "logs")
...
def test_reverse_single_short_function_not_skipped(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Short functions (<100 lines) should not be force-skipped; block pipeline is for large only."""
    from re_agent.reverse.backend.stub import StubBackend
    from re_agent.reverse.core.models import PipelineProfile
    from re_agent.reverse.orchestrator.single import reverse_single

    config = ReAgentConfig.create_default()
    config.reverse.orchestrator.block_reversal_enabled = True
    config.reverse.orchestrator.block_threshold_lines = 100
    config.reverse.output.report_dir = str(tmp_path / "reports")
    config.reverse.output.log_dir = str(tmp_path / "logs")
    ...
    assert "profile" in called_kwargs
    profile = called_kwargs["profile"]
